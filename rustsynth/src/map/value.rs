...
use crate::map::{MapRef, MapResult, ValueIter};
use crate::node::Node;
...
pub trait ValueNotArray<'map, 'elem: 'map>: Sized {
    ...
    fn append_to_map(map: &'map mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()>;
}

pub trait Value<'map, 'elem: 'map>: Sized {
    /// Retrieves the value from the map.
    fn get_from_map(map: &'map MapRef<'elem>, key: &str) -> MapResult<Self>;

    /// Sets the property value in the map.
    fn store_in_map(map: &'map mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()>;
}

impl<'map, 'elem: 'map> Value<'map, 'elem> for i64 {
    #[inline]
    fn get_from_map(map: &MapRef, key: &str) -> MapResult<Self> {
        map.get_int(key)
    }

    #[inline]
    fn store_in_map(map: &mut MapRef, key: &str, x: &Self) -> MapResult<()> {
        map.set_int(key, *x)
    }
}
...
impl<'map, 'elem: 'map> ValueNotArray<'map, 'elem> for i64 {
    ...
    #[inline]
    fn append_to_map(map: &mut MapRef, key: &str, x: &Self) -> MapResult<()> {
...
impl<'map, 'elem: 'map> Value<'map, 'elem> for Data<'elem> {
    ...
    #[inline]
    fn store_in_map(map: &'map mut MapRef, key: &str, x: &Self) -> MapResult<()> {
        map.set_data(key, x)
    }
}
...
impl<'map, 'elem: 'map> ValueNotArray<'map, 'elem> for Data<'elem> {
    ...
    #[inline]
    fn append_to_map(map: &'map mut MapRef, key: &str, x: &Self) -> MapResult<()> {
...
impl<'map, 'elem: 'map> Value<'map, 'elem> for Node<'elem> {
    #[inline]
    fn get_from_map(map: &MapRef<'elem>, key: &str) -> MapResult<Self> {
        map.get_node(key)
    }

    #[inline]
    fn store_in_map(map: &mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()> {
        map.set_node(key, x)
    }
}
...
impl<'map, 'elem: 'map> ValueNotArray<'map, 'elem> for Node<'elem> {
    ...
    #[inline]
    fn append_to_map(map: &mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()> {
...
impl<'map, 'elem: 'map> Value<'map, 'elem> for Vec<i64> {
    #[inline]
    fn get_from_map(map: &MapRef<'elem>, key: &str) -> MapResult<Self> {
        map.get_int_array(key).map(|v| v.to_vec())
    }

    #[inline]
    fn store_in_map(map: &mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()> {
        map.set_int_array(key, x.clone())
    }
}
...
impl<'map, 'elem: 'map> Value<'map, 'elem> for Vec<f64> {
    #[inline]
    fn get_from_map(map: &MapRef<'elem>, key: &str) -> MapResult<Self> {
        map.get_float_array(key).map(|v| v.to_vec())
    }

    #[inline]
    fn store_in_map(map: &mut MapRef<'elem>, key: &str, x: &Self) -> MapResult<()> {
        map.set_float_array(key, x.clone())
    }
}
...
impl<'map, 'elem: 'map> Value<'map, 'elem> for String {
    ...
    fn get_from_map(map: &'map MapRef<'elem>, key: &str) -> MapResult<Self> {
        match map.get_data(key) {
            Ok(val) => Ok(Self::from_utf8(val.to_vec())?),
            Err(err) => Err(err),
        }
    }
