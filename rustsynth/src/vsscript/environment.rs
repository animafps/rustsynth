...
use crate::{
    init_api,
    vsscript::{ScriptAPI, ScriptError},
};
...
impl Environment {
    ...
    pub fn get_variable(&self, name: &str, map: &mut Map) -> ScriptResult<()> {
        ...
        let rv = unsafe {
            ScriptAPI::get_cached().get_variable(self.handle.as_ptr(), name.as_ptr(), map.as_ptr())
        };
        if rv != 0 {
    ...
    pub fn set_variables(&self, variables: &Map) -> ScriptResult<()> {
        let rv = unsafe {
            ScriptAPI::get_cached().set_variables(self.handle.as_ptr(), variables.as_ptr())
        };
        if rv != 0 {
            Err(ScriptError::NoSuchVariable)
        } else {
